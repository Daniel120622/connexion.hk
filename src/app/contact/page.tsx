  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result?.success === true) {
        ...
        setStatus("error");
        setResponseMessage(result?.error || t("error"));
      }
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-12">
          ...
          <div className="space-y-8 order-1 lg:order-2">
            <section id="get-in-touch" className="scroll-mt-24">
              <h2 className="text-2xl font-semibold mb-4 text-gray-800">{t("getInTouch")}</h2>
              <p className="text-gray-600 mb-8 leading-relaxed">
                {t("getInTouchDescription")}
              </p>

              <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
                <h3 className="text-xl font-semibold mb-5 text-gray-800">{t("contactInfoTitle")}</h3>
            ...
            <section id="send-message" className="scroll-mt-24 bg-white rounded-xl shadow-md p-6 border border-gray-100">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                >
                  {status === "loading"
                    ? t("sendButtonLabel") === "Send Message"
                      ? "Sending..."
                      : "發送中..."
                    : t("sendButtonLabel")}
                </button>
